...
только после допуска и резервирования; её отказ не отправляет модельный запрос,
но сохраняет израсходованную попытку и включает backoff. Роли author/reviewer/reviser
...
Проверка допуска выполняется ежечасно на 17-й минуте. Это не ежечасный inference:
при cooldown/карантине работа заканчивается до установки Hermes и вызова модели.
Прежний cron раз в6ч мог приходить на несколько секунд раньше срока,
рассчитанного от завершения предыдущего ответа, и пропускать ещё6ч.
