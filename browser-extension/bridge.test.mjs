...
let listener, injections=0, grant=true, failInjection=false;
...
globalThis.chrome={
  ...
  scripting:{async executeScript(){injections++;if(failInjection)throw new Error('simulated interruption');return [{result:{ok:true,status:'send_clicked_unconfirmed',baseline_responses:0}}];}}
};
...
function reset(){for(const k of Object.keys(local.data))delete local.data[k];for(const k of Object.keys(session.data))delete session.data[k];local.data.enabled=['deepseek'];local.data.scan=true;injections=0;grant=true;failInjection=false;}
